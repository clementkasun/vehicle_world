    ...
    <div class="container-fluid">
        ...
        <div class="card card-primary">
            <div class="card-header bg-primary">
                <h3 class="card-title"><b>{{$mainNumber}}</b>. {{$row['main_name']}}</h3>
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th colspan="{{1+count($dataRow['columns'])}}" class="text-center" style="background-color: #dadada; color: #666666; font-size: 1.5em;">{{sprintf('%s %s', $titleNumber, $dataRow['title'])}}</th>
                        </tr>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @if ($attr['attr_values']['param_' . $colk]['type']=='SELECTED')
                            <td style="width: {{$colsize}}">
                                <select class="form-control">
                                    @foreach ($attr['attr_values']['param_' . $colk]['options'] as $opt)
                                    <option>{{$opt}}</option>
                                    @endforeach
                                </select>
                            </td>
                            @elseif($attr['attr_values']['param_' . $colk]['type']=='CHECKBOX')
                            <td style="width: {{$colsize}}">
